...
public class ContactMapServiceTest {
    ...
    @Test
    @DisplayName("Happy Path: Add a new contact")
    void testAddContact() {
        // Prepare & Arrange
        String name = "Anna";
        ...
        // Act¨/Perform
        service.addContact(name, number);
        Contact result = service.findContactByNumber(number);

        // Assert / Verify
        assertNotNull(result, "The contact should be found in the list");
        assertEquals(name, result.getName(), "The name should be Anna");
    }

    @Test
    @DisplayName("Negatve Case: Try to add duplicate number")
    void testDuplicateNumber() {
        // Arrange
        service.addContact("Anna", "123");
        // Act
        service.addContact("Erik", "123");
        Contact result = service.findContactByNumber("123");
                // Assert
        assertEquals("Anna", result.getName(), "The system shouldn't allow any duplicates");
    }
}
*/